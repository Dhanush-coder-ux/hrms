...
const TAG_COLORS: Record<string, any> = {
  gender:       { bg: "bg-violet-50",  text: "text-violet-700",  border: "border-violet-200",  hoverBg: "hover:bg-red-50", hoverText: "hover:text-red-600", hoverBorder: "hover:border-red-200" },
  employeeType: { bg: "bg-sky-50",     text: "text-sky-700",     border: "border-sky-200",     hoverBg: "hover:bg-red-50", hoverText: "hover:text-red-600", hoverBorder: "hover:border-red-200" },
  payType:      { bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-200", hoverBg: "hover:bg-red-50", hoverText: "hover:text-red-600", hoverBorder: "hover:border-red-200" },
  payFrequency: { bg: "bg-amber-50",   text: "text-amber-700",   border: "border-amber-200",   hoverBg: "hover:bg-red-50", hoverText: "hover:text-red-600", hoverBorder: "hover:border-red-200" },
  relationship: { bg: "bg-rose-50",    text: "text-rose-700",    border: "border-rose-200",    hoverBg: "hover:bg-red-50", hoverText: "hover:text-red-600", hoverBorder: "hover:border-red-200" },
};
      ...
      {state !== "idle" && (
        <motion.div
          ...
          className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${
            state === "saving" ? "bg-blue-50 text-blue-600 border-blue-200" :
            state === "saved"  ? "bg-emerald-50 text-emerald-600 border-emerald-200" :
...
export const BasicValues = () => {
  ...
  const persist = async (updated: StackValue) => {
    setSyncState("saving");
  ...
  const handleAdd = async (key: keyof StackValue) => {
    ...
    if (existing.some((i) => i.value === itemValue)) {
      setNewVal("");
      setActiveInput(null);
      return;
    }
    ...
    const updated = { ...data, [key]: [...existing, newItem] };
    setData(updated);
    setNewVal("");
    setActiveInput(null);
  ...
  const handleRemove = async (key: keyof StackValue, value: string) => {
    const existing = data[key] || [];
    const updated = { ...data, [key]: existing.filter((i) => i.value !== value) };
    setData(updated);
    await persist(updated);
  };
  ...
  const handleUpdateCurrency = async (opt: OptionItem) => {
    const updated = {
      ...data,
      currency: [{ label: opt.label, value: opt.value, symbol: currencySymbolMap[opt.value] }],
    };
    setData(updated);
    await persist(updated);
  };
    ...
    <main className="flex-1 min-h-screen bg-slate-50 p-6 lg:p-10">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto">
        ...
        <header className="mb-8 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Basic Settings</h1>
            <p className="text-sm text-slate-500 mt-1">Configure global system variables and dropdown options.</p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <section className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2.5">
              ...
              <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest">System Defaults</h2>
            </div>
            ...
            <div className="divide-y divide-slate-100">
              <div className="px-6 py-5 flex items-center justify-between gap-6">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Global currency used across all records</p>
                </div>
                ...
                <div className="flex items-center gap-4 shrink-0">
                  <div className="w-9 h-9 rounded-xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-600 font-bold text-base">
                    {currentCurrency?.symbol || currencySymbolMap[currentCurrency?.value || ""] || "—"}
                  </div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) => {
                        const opt = currencyOptions.find((o) => o.value === e.target.value);
          ...
          <section className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-2.5">
              ...
              <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Dropdown Configurations</h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y divide-slate-100 md:divide-y-0 md:border-b-0">
              {FIELD_CONFIGS.filter((f) => !f.isDefault).map((config, idx) => {
                ...
                const isLastTwo = idx >= FIELD_CONFIGS.filter(f => !f.isDefault).length - 2;
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-slate-400 mt-0.5 uppercase tracking-wider font-medium">
                          {fieldItems.length} option{fieldItems.length !== 1 ? "s" : ""}
                        </p>
                      </div>
                      <button
                        onClick={() => { setActiveInput(config.key); setNewVal(""); }}
                    ...
                    <div className="flex flex-wrap gap-2 min-h-9 content-start">
                      <AnimatePresence>
                        {fieldItems.map((item) => (
                          ...
                          >
                            {item.label}
                            <button onClick={() => handleRemove(config.key, item.value)} className="opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:bg-red-100 -mr-0.5">
                              <X size={10} />
                            </button>
                          </motion.span>
                        ))}
                          ...
                          <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: "auto" }} exit={{ opacity: 0, width: 0 }} className="overflow-hidden">
                            <input
                              ...
                              onKeyDown={(e) => {
                                ...
                                if (e.key === "Escape") { setActiveInput(null); setNewVal(""); }
                              }}
                              onBlur={() => newVal.trim() ? handleAdd(config.key) : setActiveInput(null)}
