...
import { Search, Download, CreditCard, Users, PieChart } from 'lucide-react';
...
const PayrollComponents = ({ data: initialData }: PageProps) => {
  ...
  const [payrollData, setPayrollData] = useState<PayrollData[]>(initialData || []);
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredData.map(r => `${r.employee},${r.salary},${r.tax},${r.net},${r.status},${r.department}`).join("\n");
  ...
  const columns = [
    ...
    { 
      ...
      render: (row: PayrollData) => (
        <span className="font-mono font-medium">${row.net.toLocaleString()}</span>
      ) 
    },
    { 
      header: 'Status', 
      ...
      render: (row: PayrollData) => (
        <span className={`px-2.5 py-1 rounded-full text-xs font-semibold shadow-sm ${
          ...
          row.status === 'Processing' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'
        }`}>
          {row.status}
        </span>
      )
...
const ActionToClickRow = (row: PayrollData) => {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="animate-pulse text-indigo-600 font-medium text-lg">Loading Payroll Dashboard...</div>
    </div>
  );
    ...
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
          <div>
            <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight">Payroll Hub</h1>
            <p className="text-gray-500 mt-1 text-lg">Financial overview for the current billing cycle</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <StatCard icon={<CreditCard className="text-indigo-600" />} label="Total Disbursement" value={`$${stats.totalNet.toLocaleString()}`} />
          <StatCard icon={<Users className="text-emerald-600" />} label="Total Employees" value={stats.totalEmployees.toString()} />
          <StatCard icon={<PieChart className="text-amber-600" />} label="Pending Approvals" value={stats.pendingCount.toString()} />
        </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6 flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            name='serchterm'
             ...
             icon={<Search size={16} />} 
             in_PlaceHolder='Search by employee name...'
             value={searchTerm} 
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          {filteredData.length === 0 && (
            <div className="p-20 text-center text-gray-400">No records found matching your criteria.</div>
          )}
        </div>
      </div>
    </div>
