...
export const Events = () => {
  ...
  const initialFormState: EventFormData = {
    ...
    plan: { plan_type: "", details: "" }, description: "",
  };
  ...
  const handleSave = async () => {
    const newEvent = { ...formData, event_id: formData.event_id || crypto.randomUUID() };
    try {
      const response = await fetch(API_URL, {
        method: "POST",
  ...
  const handleDateClick = (arg: any) => {
    ...
    const eventsOnDate = EventDatas.filter((event) => event.date === clickedDate);
    setSelectedEvent(eventsOnDate.length > 0 ? eventsOnDate[0] : null);
    setShowEdit(false);
  };
      ...
      <div className="lg:w-1/3 space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <ul className="text-sm text-gray-600">
            {EventDatas.slice(0, 5).map((ev) => (
              <li key={ev.event_id} className="mb-1">• {ev.event_title}</li>
            ))}
          </ul>
        </div>
      </div>
          ...
          <div className="bg-white w-full max-w-md p-6 rounded-xl shadow-lg">
            ...
            {EventDatas.filter((ev) => ev.date === selectedDate).length > 0 ? (
              <ul className="space-y-2">
                {EventDatas.filter((ev) => ev.date === selectedDate).map((ev) => (
                  <li key={ev.event_id} className="p-3 border rounded-lg">
            ...
            <div className="flex justify-between mt-6">
              ...
              <button onClick={() => setSelectedDate(null)} className="px-4 py-2 bg-gray-200 rounded-lg">
                Close
              </button>
            </div>
          </div>
