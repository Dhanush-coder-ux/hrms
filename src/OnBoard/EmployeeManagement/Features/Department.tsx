...
export const Department = () => {
  ...
  const totalEmployees = departments.reduce((acc, curr) => acc + (Number(curr.emp_count) || 0), 0);
  const avgSize = departments.length > 0 ? Math.round(totalEmployees / departments.length) : 0;
  ...
  return (
    <div className="p-8 bg-[#f9fafb] min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-6">
        <div>
          <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tight">Department Hub</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          { label: "Total Depts", value: departments.length, icon: <LayoutGrid size={20}/>, color: "text-blue-600" },
          { label: "Total Staff", value: totalEmployees, icon: <Users size={20}/>, color: "text-emerald-600" },
          ...
          <div key={i} className="bg-white p-6 rounded-4xl border border-gray-100 shadow-sm flex items-center justify-between">
            ...
            <div className="p-4 bg-gray-50 rounded-2xl text-gray-400">{stat.icon}</div>
          </div>
        ))}
      </div>
        ...
        <DepTable
          ...
          getStatusColor={(status: string) => {
            switch (status) {
              ...
              case "Completed": return "text-violet-600 bg-violet-50 border-violet-100";
              default: return "text-gray-400 bg-gray-50 border-gray-100";
            }
          }}
          onEdit={(row: any) => navigate(`/EmployeeManagement/departmentProfile/${row.id}`)}
        />
      </div>
    </div>
  );
